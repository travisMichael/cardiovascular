...
def train_svm(path, with_plots):
    ...
    if not with_plots:
        print('Training SVM...')
        model_1 = train_and_time(svm.SVC(kernel='linear', cache_size=400, probability=True), x_train, y_train)
        model_2 = train_and_time(svm.SVC(kernel='poly', cache_size=400, probability=True), x_train, y_train)
